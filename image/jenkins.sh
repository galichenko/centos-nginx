...
if [[ $# -lt 1 ]] || [[ "$1" == "--"* ]]; then
  ...
  if [ ! -d "/var/jenkins_home/users" ];
   then
    ...
    rm -f /var/jenkins_home/users/admin/config.xml
    cp -f /tmp/config.xml /var/jenkins_home/users/admin/
    chown jenkins:jenkins /var/jenkins_home/users/admin/config.xml
  fi
